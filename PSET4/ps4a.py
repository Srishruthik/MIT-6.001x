...
WORDLIST_FILENAME = "/Users/shruthik/Documents/Coding/Courses/PSET4/words.txt"
...
def isValidWord(word, hand, wordList):
    ...
    check_letter = set(list(word)) <= set(total.keys())

    for letter in word:
        if letter in total.keys():
            total[letter] -= 1

    check_value = all(i >= 0 for i in total.values())
    ...
    while calculateHandlen(hand) > 0:
        ...
        if guess == '.':         
            ...
            if isValidWord(guess, hand, wordList) == False:             
                ...
                print('"'+guess+'"', "earned a total of", getWordScore(guess, n), "points. Total:", score, "points", '\n')
                hand = updateHand(hand, guess)               
   
    if guess == '.':
        print('Goodbye! Total score:', score, 'points.')
    else:
        print('Rn out of letters. Total score:', score, 'points.')  
...
def playGame(wordList):
    ...
    # TO DO ... <-- Remove this comment when you code this function
    while True:
        user_input = str(input('Enter n to deal a new hand, r to replay the last hand, or e to end game: ')).lower().strip()
        if user_input == 'n':
